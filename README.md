...
**30 秒定位：** 一个零后端、零构建步骤的静态互动测试。核心不在页面数量，而在 12 维画像建模、可复用匹配引擎和内容数据分离。

**建议 2 分钟演示：** 完成几道题 -> 展示实时进度与结果页 -> 打开 `idol-data.mjs` 说明题库/画像如何配置 -> 运行 Node 原生测试验证匹配逻辑。

**工程亮点：**

- 题库、人物画像与通用评分引擎分离，新增结果不需要改页面流程。
- 纯 ESM + Node 内置测试，无框架依赖，适合静态托管。
- 通过多维 H/M/L 向量做距离匹配，比单个总分更容易解释结果来源。
...
本项目是非商业化的粉丝向交互 Demo；人物名称与图片权利归各自权利方所有，公开部署前应确认所用图片来源和授权范围。
